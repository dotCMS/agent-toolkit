...
**`page_verify` does not apply.** It renders dotCMS's own VTL output, which a headless
...
2. **The app renders it.** Run the dev server and request the route. Every content
   type on the page must resolve to a component.

## Triage

| Symptom | Cause |
|---|---|
| Slot renders "no component" / empty | component-map key ≠ content type variable, case-exact ([01](01-component-contract.md)) |
