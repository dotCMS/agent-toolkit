...
Build from this **PLAN.md** (structure + tagged facts) + **`DESIGN.md`** (visual system: tokens + rationale), following the build references in **`reference/build/`** (start at `build/README.md`, which routes you to `core/` plus either `vtl/` or a framework branch such as `nextjs/`, per §3b). **`[PLACEHOLDER]` renders as a visible TODO marker, never an invented value; `[n/a]` fields/sections are removed entirely, not flagged.** `[ai-draft]` copy needs human approval before launch. Verify every page type per your branch's verify step (`build/vtl/05` or `build/nextjs/05`) before done.
